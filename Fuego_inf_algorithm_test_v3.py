...
t1 = UTCDateTime(2018, 5, 20, 0, 0, 0) #the format is year:day_of_the_year:month
t2 = t1 + 60*60*24
st_day = Stream()
...
for x in range(0,len(on_off)):
    event = trace[on_off[x,0]-1000:on_off[x,1]+2000]    
...
#    trough_r = peaks_m/trough_m
    ...
    if len(event)/(sr*60) < 1 :
        ...
        print(UTCDateTime(start + on_off[x,0]/sr ),'trem')
        plt.figure()
        plt.plot(event,color='r')
        plt.title(UTCDateTime(start + on_off[x,0]/sr ))
#    print(len(event)/(50*60),'mins')
